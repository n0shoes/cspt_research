// Framework detection — ported from Caido plugin framework-detector.ts
// Weighted signal detection for 10 frameworks
...
const DETECTION_ORDER = [
  "nextjs", "remix", "nuxt", "sveltekit", "solidstart",
  "astro", "angular", "ember", "vue-router", "react-router",
];

DoctorScan.detectFramework = function(body) {
  const results = [];
...
// Also return all detected frameworks (for secondary detections)
DoctorScan.detectAllFrameworks = function(body) {
  const results = [];
  for (const framework of DETECTION_ORDER) {
    const signals = FRAMEWORK_SIGNALS[framework];
    if (!signals) continue;
    ...
    if (totalWeight >= 30) {
      results.push({ framework, confidence: Math.min(totalWeight, 100), signals: matchedSignals });
    }
  }
...
DoctorScan.getContentType = function(url, mimeType) {
  if (!mimeType && url) {
