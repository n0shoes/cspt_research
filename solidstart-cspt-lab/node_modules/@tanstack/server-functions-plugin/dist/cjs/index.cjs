...
const defaultManifestFilename = "node_modules/.tanstack-start/server-functions-manifest.json";
      ...
      Object.fromEntries(
        Object.entries(d).map(([id, fn]) => [
          id,
          {
            ...
            importer: () => opts.importer ? opts.importer(fn) : import(fn.extractedFilename)
          }
        ])
      )
    );
  };
  const manifestFilename = opts.manifestOutputFilename || defaultManifestFilename;
  ...
  return {
    client: [
      ...
      }),
      {
        ...
        generateBundle() {
          node_fs.mkdirSync(path.join(ROOT, path.dirname(manifestFilename)), {
            recursive: true
          });
